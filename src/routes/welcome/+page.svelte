...
<div class="wrap">
  ...
  <svg class="nodes-svg" aria-hidden="true" viewBox="0 0 900 600" preserveAspectRatio="xMidYMid slice">
    <defs>
      <filter id="glow">
  ...
  <!-- corner watermark -->
  <div class="watermark" class:show={phase >= 3}>
    系统建模与仿真智能体 v1.0
  </div>
</div>
...
<style>
  ...
  .grid-layer {
    ...
    inset: 0;
    background-image:
      linear-gradient(rgba(80,140,255,0.10) 1px, transparent 1px),
  ...
  .node-line {
    stroke: rgba(80,180,255,0.30);
    stroke-width: 1.5;
    stroke-dasharray: 8 4;
    animation: dash-flow 3s linear infinite;
  }
  .node-line:nth-child(2) { animation-delay: -0.8s; }
  .node-line:nth-child(3) { animation-delay: -1.5s; }
  .node-line:nth-child(4) { animation-delay: -2.2s; }
  .node-line:nth-child(5) { animation-delay: -0.4s; }
  .node-line:nth-child(6) { animation-delay: -1.1s; }
  @keyframes dash-flow {
  ...
  .node-dot:nth-child(odd) { animation-delay: -1.2s; }
  @keyframes node-pulse {
    from { opacity: 0.55; r: 6; }
    to   { opacity: 1; r: 9; }
  }
  .node-label {
  ...
  .cta-btn:hover {
    ...
    box-shadow: 0 0 40px rgba(0,200,240,0.50), 0 8px 32px rgba(0,0,0,0.50);
